      ...
      <div class="container-left">
        ...
        <div class="detail-list-wrap">
          ...
          <ul class="bank-list multi-line">
            <li v-for="contract in contractList" @click="getThisDetail(contract.id)">
              <el-checkbox v-model="contract.checked"></el-checkbox>
              <div class="item  item-lf">
                <div class="item-tel">{{contract.contractName}}</div>
                <div class="item-bank">{{contract.partyBName}}</div>
              </div>
              <div class="item item-rt">
                <div class="item-time">{{contract.createTime | formatDate("yyyy-MM-dd")}}</div>
                <div class="item-status" v-bind:class="contract.status|applyStatus().color">{{contract.statusDesc}}</div>
              </div>
            </li>
          </ul>
        </div>
...
<script>
  export default {
    ...
    methods: {
      getList() {
        this.$axios .get("scontract/query")
        .then(response => {
            this.contractList = response.rows;
        });
      },
      getDetail(pid) {
        this.$axios .get("scontract/get/"+pid)
        .then(response => {
