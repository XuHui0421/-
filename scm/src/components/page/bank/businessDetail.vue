      ...
      <div class="container-left">
        <div class="detail-list-wrap">
          <ul class="company-list multi-line">
            <li v-for="company in companyList" @click="getThisDetail(company.portraitId)">
              <el-checkbox v-model="company.checked"></el-checkbox>
              <div class="item  item-lf">
                <div class="item-tel">{{company.name}}</div>
                <div class="item-bank">{{company.contract}}</div>
              </div>
              <div class="item item-rt">
                <div class="item-time">{{company.startDate}}</div>
                <div>{{company.contractTel}}</div>
              </div>
            </li>
          </ul>
        </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getDetail(pid) {
        this.$axios.get('/menter/get/portrait/' +pid)
          .then((response) => {
      ...
      getThisDetail(portraitId) {
        for (let i = 0; i < this.companyList.length; i++) {
          if (this.companyList[i].portraitId == portraitId) {
