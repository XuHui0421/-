<template>
  <div class="container" ref="container">
    ...
    <div class="finance-process-box">
          <finance-process :status="detailInfo.status" :dataWayType="detailInfo.dataWayType"></finance-process>
    </div>
      ...
      <div class="container-right" >
        <div class="detail-header">
          ...
          <div class="operate-list">
            <el-button type="primary" size="mini" round  v-if="[15,16].indexOf( detailInfo.status) != -1" @click="loanDialog = true">
              放款登记
            </el-button>
            <el-button type="primary" size="mini" round  v-if="detailInfo.status == 20" class="p-re">
              回款登记
              <input type="file" name="file" class="input-file" @change="payback($event)" ref="payback">
            </el-button>
    ...
    <el-dialog title="放款登记" :visible.sync="loanDialog" width="30%">
        <el-form :model="loanForm" :rules="loanFormRules" label-position="left"  label-width="80px" class="form-center" ref="loanForm" >
            <el-form-item label="放款时间"  prop="loanTime">
                <el-date-picker v-model="loanForm.loanTime" type="date" placeholder="请选择放款时间" value-format="yyyy-MM-dd"></el-date-picker>
            </el-form-item>
            <el-form-item label="放款人" prop="loanPerson">
                <template>
                    <div style="width: 220px;">
                        <el-input type="text" v-model="loanForm.loanPerson" auto-complete="off" placeholder="请输入放款人"></el-input>
                    </div>
                </template>
        ...
        <div slot="footer" class="dialog-footer">
            <el-button @click="loanDialog = false">取 消</el-button>
            <el-button type="primary" @click="loan()">确 定</el-button>
        </div>
    </el-dialog>
  </div>
</template>
...
import FinanceProcess from '../financeProcess';
  export default {
    ...
    data() {
      return {
        loanDialog: false,//放款登记
        loanForm: {
            loanTime: '',
            loanPerson: ''
        },
        loanFormRules: {
            loanTime: {required: true, message: '请选择放款时间',},
            loanPerson: {required: true, message: '请输入放款人',},
        },
        paybackDialog: false,//回款登记
        paybackForm: {
            loanTime: '',
            loanPerson: ''
        },
        paybackFormRules: {
            loanTime: {required: true, message: '请选择回款时间',},
            loanPerson: {required: true, message: '请输入回款人',},
        },
        financeList: [],
        totalNum: 0,
        activeTab: "first",
    ...
    methods: {
      getList() {
        this.$axios.get("mloan/query/bank")
        .then(response => {
            this.financeList = response.rows;
        });
      },
      getDetail(pid) {
        this.$axios.get("mloan/get/" + pid)
        .then(response => {
      ...
      loan(){//放款
        this.$refs['loanForm'].validate((valid) => {
            if(valid){
                this.$axios.post("/mloan/loan/regist?id="+ this.$route.params.id+'&loanTime='+ this.loanForm.loanTime+'&loanPerson='+this.loanForm.loanPerson)
                .then((response) => {
                   this.$Message.success('放款登记成功!');
                   this.loanDialog = false;
                   this.detailInfo.status = 20;
                   this.detailInfo.statusDesc = "已放款";
                });
            }   
        })
      },
      payback(){//回款
         if (this.$refs.payback.files.length !== 0) {
          var file = new FormData();
          file.append('file', this.$refs.payback.files[0])
          this.$axios.post('mloan/core/payment?id='+this.$route.params.id, file)
            .then((response) => {
                this.$Message.success('回款成功!');
                this.detailInfo.status = 25;
                this.detailInfo.statusDesc = "已回款";
            })
        }
      }
    }
